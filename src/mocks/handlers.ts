...
import { Project, Board, Column, Task, UpdateTaskDTO } from '../types';
...
export const handlers = [
  ...
  http.get(`${API_URL}/boards/:boardId`, () => {
    return HttpResponse.json({
      board: mockBoard,
      columns: mockColumns,
      tasks: mockTasks,
  ...
  http.patch(`${API_URL}/tasks/:taskId`, async ({ request, params }) => {
    const { taskId } = params;
  ...
  http.delete(`${API_URL}/tasks/:taskId`, ({ params }) => {
    const { taskId } = params;
