...
import styles from './styles/createProjectDialog.module.css';
...
export function CreateProjectDialog({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (projectName.trim()) {
      onCreateProject(projectName.trim(), projectDescription.trim(), selectedIcon);
      setProjectName('');
    ...
    <div className={styles.overlay} onClick={handleClose}>
      <div className={styles.dialog} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          ...
          {workspaceName && (
            <p className={styles.workspaceInfo}>em {workspaceName}</p>
          )}
            ...
            <div className={styles.previewCard}>
              <div className={styles.previewIcon}>
                <svg 
                  ...
                  viewBox="0 0 24 24" 
                  fill="none" 
                  style={{ color: workspaceColor || '#162456' }}
                >
                  <path d={ICONS.find(icon => icon.id === selectedIcon)?.svg || ICONS[0].svg} fill="currentColor" />
                </svg>
              </div>
